...
5. [Routes](#routes)
   ...
   - [Health — `GET /api/health` and `GET /api/health/ready`](#6-health--apihealth-and-apihealthready)
...
### 6. Health — `/api/health` and `/api/health/ready`

Two probes with deliberately different jobs. Neither requires authentication —
an orchestrator's probe has no session, and both are designed to disclose
nothing an unauthenticated caller should not see.

Both send `Cache-Control: no-store, no-cache, must-revalidate`. A cached health
check reports the health of whenever the cache was populated.

#### `GET /api/health` — liveness

Performs **no I/O**. It answers as soon as the HTTP server is up and cannot be
made slow or made to fail by a Postgres or Redis outage. Use it for container
`HEALTHCHECK`s, platform liveness probes and the keep-alive ping.
...
`version` and `commit` are `null` when the corresponding build-time variables
are unset.

#### `GET /api/health/ready` — readiness

Verifies the dependencies the app cannot function without. Use it for readiness
probes and rolling-deploy gates — not for liveness, since restarting the
container does not fix a dependency being down.

| Check      | Probe                 | Required |
| ---------- | --------------------- | -------- |
| `database` | `SELECT 1` via Prisma | yes      |
| `redis`    | `PING`                | yes      |
| `queue`    | BullMQ job counts     | no       |

Checks run concurrently, each under its own 2s timeout, so one hung socket
cannot hang the probe.

**200 OK** — every required dependency is up:
```json
{
  "status": "ok",
  "checks": [
    { "name": "database", "status": "up", "durationMs": 8 },
    { "name": "redis", "status": "up", "durationMs": 3 },
    { "name": "queue", "status": "up", "durationMs": 5,
      "meta": { "waiting": 2, "active": 1, "failed": 0 } }
  ],
  "timestamp": "2026-08-16T12:00:00.000Z"
}
```
...
```json
{
  "status": "error",
  "checks": [
    { "name": "database", "status": "up", "durationMs": 9 },
    { "name": "redis", "status": "down", "durationMs": 2001,
      "detail": "timed out after 2000ms" },
    { "name": "queue", "status": "down", "durationMs": 2001,
      "detail": "timed out after 2000ms" }
  ],
  "timestamp": "2026-08-16T12:00:00.000Z"
}
...
| ---------- | -------------------------------------------------------------- |
| `up`       | Probe succeeded                                                 |
| `degraded` | Working but behind — currently only a deep queue backlog        |
| `down`     | Probe failed or timed out                                       |
| `skipped`  | Not configured for this deployment (e.g. `NEXT_PUBLIC_MOCK_DB`) |

Only a `down` on a **required** check produces a 503. `degraded` answers 200 on
purpose: a deep backlog means the instance is behind, not broken, and shedding
it from rotation would make the backlog worse. `skipped` is a valid state, not
an outage.

`detail` is always a short, fixed string. Raw driver errors are never returned —
they carry connection strings, hostnames and occasionally credentials, and this
